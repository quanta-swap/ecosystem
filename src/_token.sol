...
contract TradesPerDayToken is IZRC20 {
    ...
    /*──────── constructor ────────*/
    constructor(string memory name_, string memory symbol_, uint64 fixedSupply) {
        _name = name_;
        _symbol = symbol_;
    ...
    function transferBatch(
        ...
        /* calls transferFromBatch with src = msg.sender */
        return _transferFromBatch(msg.sender, dst, wad);
    }
    ...
    function transferFromBatch(
        ...
        return _transferFromBatch(src, dst, wad);
    }

    function _transferFromBatch(
        address src,
        address[] calldata dst,
        uint64[] calldata wad
        ...
        for (uint256 i; i < len; ++i) {
            ...
            require(_transferFrom(src, dst[i], wad[i]), "xfer fail");
        }
        return true;
    }
}